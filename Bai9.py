# Quy dao vat the ve duong bao tuong duong
from cv2 import cv2
import time
import numpy as np
...
while True:
    ...
    if len(contours) > 0:
        tmp_c = contours [0]
        (cx,cy),cr = cv2.minEnclosingCircle(tmp_c)#Ve mot duong tron bao quanh voi toa do tam x,y va ban kinh r
        res = cv2.circle(img_clone,(int (cx),int (cy)),int (cr*2/3),(0,0,255),2)
        arr_point = np.append(arr_point,(int (cx),int (cy)))
        if len(arr_point) > 150: #Neu chieu dai qdao >150 thi se xoa 2 phan tu dau tien cua x,y
